...
3. Deploy new callables: `teamCommand`, `getPublicTeamPage`, `getTeamInvitationPreview`, `getTeamInsights`, `submitTeamContact`, `manageTeamContacts`, `getTeamConversations`; and `teamVoiceWebhook`.
...
6. Configure `ELEVENLABS_TEAM_WEBHOOK_SECRET` and the provider's post-call transcription webhook to the deployed `teamVoiceWebhook` endpoint. Preserve the existing ElevenLabs API key/agent configuration. Shared live voices additionally require `ELEVENLABS_TTS_VOICE_OVERRIDES_ENABLED=true` and provider-side override support. Test a real external call and confirm its signed callback and duration.
...
- `ELEVENLABS_TEAM_WEBHOOK_SECRET` does not exist yet. `teamVoiceWebhook` was deliberately excluded from deployment. Its secret is bound directly to that endpoint, so it no longer blocks unrelated core-function deployments. Provider setup and verified call metrics remain pending.
...
Latest passing checks: **479 browser tests, 68 Firestore/backend integration tests, 10 Storage security tests, and 9 team policy tests**. Regressions cover avatar-menu discoverability for unknown/denied/allowed quotas, failed-versus-empty invitation states, retries, independent check failures, stale requests, and account changes. Run the Firestore and Storage emulator suites sequentially because they share ports.
